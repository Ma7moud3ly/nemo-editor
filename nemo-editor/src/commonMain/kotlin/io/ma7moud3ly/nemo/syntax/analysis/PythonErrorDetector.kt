...
class PythonErrorDetector : ErrorDetector() {
    ...
    override fun findDuplicateKeywords(tokens: List<Token>): List<SyntaxError> {
        ...
        while (i < tokens.size) {
            ...
            if (token.type == TokenType.KEYWORD) {
                ...
                // Only flag same keyword twice in a row (def def, class class)
    ...
    override fun findUnclosedStrings(tokens: List<Token>): List<SyntaxError> {
        ...
        tokens.forEach { token ->
            if (token.type == TokenType.STRING) {
                ...
                // Skip f-strings, r-strings, b-strings - they're handled by tokenizer
                if (text.startsWith("f\"") || text.startsWith("f'") ||
                    ...
                    // Don't check prefix strings, they're complex
                    return@forEach
                }
